#!/bin/bash
# ARGUS full headline run — all flags on, 15 episodes, eval n=100.
# Pushes the GPU to peak from episode 1: solver_k=14 (max from start,
# no ramp), solver_mnt=352, epiplexity_samples=96. Eval bumped to
# n=100 for tighter Wilson CI on pass@k.
# Logs to logs/training/v34_full_<timestamp>.log via tee.
# Estimated wall time: ~6.5-7h on RTX 5070 Ti Laptop.
...
# Launch screenshot monitor in background — captures the full log as
# a Zinc-950 PNG at start / 50% / 70% / end. Saves to
# logs/screenshots/${NAME}/{1_start,2_mid1,3_mid2,4_end}.png.
...
# Give monitor up to 30s to capture the final screenshot, then move on.
wait_loop=0
